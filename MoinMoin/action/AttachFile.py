"""
    ...
    $Id: AttachFile.py,v 1.27 2002/03/08 19:40:08 jhermann Exp $
"""
...
def send_hotdraw(pagename, form):
    now = time.time()
    pubpath = config.url_prefix + "/applets/TWikiDrawPlugin"
    drawpath = getAttachUrl(pagename, form['drawing'].value + '.draw')
    gifpath = getAttachUrl(pagename, form['drawing'].value + '.gif')
...
def send_uploadform(pagename, form):
    ...
    if form.getvalue('drawing', None):
        return send_hotdraw(pagename, form)
...
def execute(pagename, form):
    ...
    elif form.has_key('filepath'):
        save_drawing(pagename, form)
        webapi.http_headers()
        print "OK"
...
def save_drawing(pagename, form):
    filename = form['filepath'].filename
    content = form['filepath'].value
